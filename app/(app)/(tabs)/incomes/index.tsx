...
import { deleteIncome, incomeConstraints } from '@/lib/api'
...
export default function IncomesScreen() {
  ...
  const handleRestore = async (item: WithPending<Income>) => {
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={cn("px-3.5 py-2.5 rounded-[10px] h-9 items-center justify-center", filters.parcelId === item.id ? "bg-green-600 dark:bg-green-500" : "bg-accent")}
              >
              ...
              ListEmptyComponent={
                !loading ? (
                  <View className="items-center py-12">
                    ...
                    <Text className="text-muted-foreground mt-3">{t.noIncomes}</Text>
                  </View>
                ) : null
              }
              renderItem={({ item }) => {
                ...
                return (
                  <View className="flex-row items-center p-3.5 rounded-xl border border-border mb-2">
                    ...
                    <View className="flex-1">
                      ...
                      {unitPrice ? <Text className="text-[11px] text-muted-foreground mt-0.5">{t.unitPrice}: {formatMADDecimal(unitPrice)} MAD/{item.unit || 'u'}</Text> : null}
